...
len(arr)              = length(arr)
figr                  = go.Figure
plot(fig::Py)         = (pyimport("plotly.offline").plot(fig, include_mathjax="cdn"); return nothing)
...
line(x, y; kwargs...) = go.Figure().add_trace(go.Scatter(x=collect(x), y=y; kwargs...))
line(y; kwargs...) = go.Figure().add_trace(go.Scatter(y=y; kwargs...))

line(fig::Py, x, y; kwargs...) = fig.add_trace(go.Scatter(x=collect(x), y=y; kwargs...))
line(fig::Py, y; kwargs...) = fig.add_trace(go.Scatter(y=y; kwargs...))

function figFormat(fig; font_size=30, tick_width=3, margin=zeros(Int, 4), tick_bold=true)
    fig.update_layout(
        font_family   = "Arial",
        font_color    = "black",
        font_size     = font_size,
        ...
        margin=Dict(:t=>margin[1], :b=>margin[2], :l=>margin[3], :r=>margin[4]) 
    )
    tick_fix = tick_bold ? "<b>" : ""
    fig.update_xaxes(tickprefix=tick_fix, ticksuffix=tick_fix, ticks="inside", tickwidth=tick_width,
                     showgrid=false, showline=true, linewidth=tick_width, exponentformat="power",
                     linecolor="black", mirror="ticks", zeroline=false)
    fig.update_yaxes(tickprefix=tick_fix, ticksuffix=tick_fix, ticks="inside", tickwidth=tick_width,
                     showgrid=false, showline=true, linewidth=tick_width, exponentformat="power",
                     linecolor="black", mirror="ticks", zeroline=false)
   return fig 
end

function pfig(nrows=1, ncols=1; returnfig=false, figsize=(6, 4), fontsize=18, fontfamily="Times New Roman", texfont="cm", framewidth=1, ticksize=5, label_pos=(0.02, 0.91), kwargs...)
    ...
    if nrows*ncols > 1
        ...
        for i in 1:nrows, j in 1:ncols
            ax[(j-1)*nrows+i].text(label_pos[1], label_pos[2], labels[(i-1)*ncols+j], transform=ax[(j-1)*nrows+i].transAxes)
